...
result_codes device_read_dht11(struct usb_serial_port *port, const char* device_name, char* buffer)
{
    ...
    if(strstr(device_name, "_temperature"))
        sprintf(buffer, "%d\n", data.temperature);
    else if(strstr(device_name, "_humidity"))
        sprintf(buffer, "%d\n", data.humidity);
    else if(strstr(device_name, "_heat_index"))
        sprintf(buffer, "%d\n", data.heat_index);
    else
        return DEVICE_READ_FAILED;
...
struct device** device_add_dht11(struct class *device_class, uint16_t id, uint8_t *devices_objects_count)
{
    ...
    ret = alloc_chrdev_region(&dev, 0, 3, DHT11_NAME);
    ...
    devices_objects[0] = device_create(device_class, NULL, dev, NULL, "%s_%d_temperature", DHT11_NAME, id);
    devices_objects[1] = device_create(device_class, NULL, dev + 1, NULL, "%s_%d_humidity", DHT11_NAME, id);
    devices_objects[2] = device_create(device_class, NULL, dev + 2, NULL, "%s_%d_heat_index", DHT11_NAME, id);

    if (!devices_objects[0] || !devices_objects[1] || !devices_objects[2]) 
    {
